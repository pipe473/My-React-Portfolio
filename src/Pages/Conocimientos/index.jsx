import React from 'react';
import './Conocimientos.scss';

function Conocimientos(props) {

    return(

        <div className="programas">

            <h1>Conocimientos</h1>

            <div className="row1">
                <a className="item" href='#' target="React.JS"></a><i className="fab fa-react"></i>
                <a className="item" href='#' target="ANGULAR2"></a><i className="fab fa-angular"></i>
                <a className="item" href='#' target="HTML5"></a><i className="fab fa-html5"></i>
                <a className="item" href='#' target="CSS3"></a><i className="fab fa-css3-alt"></i>
                <a className="item" href='#' target="JS"></a><i className="fab fa-js-square"></i>
            </div>

            <div className="row2">
                <a className="item" href='#' target="SASS"></a><i class="fab fa-sass"></i>
                <a className="item" href='#' target="NODE.JS"></a><i class="fab fa-node"></i>
                <a className="item" href='#' target="BOOTSTRAP"></a><i class="fab fa-bootstrap"></i>
                <a className="item" href='#' target="DOCKER"></a><i class="fab fa-docker"></i>
                <a className="item" href='#' target="YARN"></a><i class="fab fa-yarn"></i>
            </div>           
        </div>
    );    
}

export default Conocimientos;


